...
def ruleOnRecpt(Quote, rebTypes):
    ...
    if rebTypes.IS_CUSTGP: #rebate type is customer group
        ...
        if agrType == "1": #offer agreement type = sold-to
            ...
            Quote.GetCustomField("BO_CF_REBATE_RECIPIENT").Required = True #3008 - 17/09/2021
...
def rebateScaleVisibility(rebateScaleTable, rebateType, Quote):
    ...
    if rebateType != "":
        ...
        if rebTypes:
            ...
            #START BUG 3008 - Show/Hide CUST_HIER in quote table BO_SOLDTO
            if rebTypes.IS_CUSTGP:
                Quote.QuoteTables["BO_SOLDTO"].GetColumnByName("CUST_HIER").AccessLevel = AccessLevel.Hidden
            else:
                Quote.QuoteTables["BO_SOLDTO"].GetColumnByName("CUST_HIER").AccessLevel = AccessLevel.Editable
            #END BUG 3008 - Show/Hide CUST_HIER in quote table BO_SOLDTO
        else:
...
#Data for Redundant Goods---------------------------------------------------------------> #++ Bug 3280 Brian 21/09/2021
            elif variable["TYPE"] == "G":
                #get new row
                newRow = table.AddNewRow()
                #update new row
                newRow["OBJECT"]	   = obj
...
def getRebateCondition(condType,#condition code; ex: Z*%, Z*Q
    ...
    AND IS_RLSH		= '{reelSh}' """.format(condCode= condType,
                                            ...
                                            endObj	= isEndObj,
                                            cust	= isCust))
    return table.TABLE_NUM if table else ""
...
def getMultiplierKeys(erpTable,
						...
						cat,			#Brand Category
						region,			#Region
						surcharge,		#surcharge row
						conditionKey,
						tableNum,
						condType

						usage,		#Usage of the condition table; constant "E"
						tableNum,	  	#condition table
						app,		  	#Application; constant"V"
						condType, 	#condition type
						variableKey,  #variable key
	...
	for line in reversed(erpTable):
		...
		#no variable key built yet
		if len(multiplierKeys) <= 0:
			Trace.Write("[TRACE] " + line.SAP_FIELD)
			if line.SAP_FIELD == "ZZVCENDUSEOBJCT": #end use object
				multiplierKeys.append(endObj.ljust(line.LENGTH))
			elif line.SAP_FIELD == "VKORG": #sales organisation
				multiplierKeys.append(sOrg)
			elif line.SAP_FIELD == "VTWEG": #Distribution Channel
				multiplierKeys.append(distCh)
			elif line.SAP_FIELD == "SPART": #Division
				multiplierKeys.append(divOrg)
			elif line.SAP_FIELD == "KUNNR": #Sold-to party
				multiplierKeys.append(soldTos.zfill(line.LENGTH))
			elif line.SAP_FIELD == "ZZCUSZE": #End user
				multiplierKeys.append(endCust.zfill(line.LENGTH))
			elif line.SAP_FIELD == "ZZHIEZU01": #CustomerHierarchy01
				multiplierKeys.append(" ".zfill(line.LENGTH))
			elif line.SAP_FIELD == "ZZVCSBRAND2": #sales brand 2
				multiplierKeys.append(" ".ljust(line.LENGTH))
			elif line.SAP_FIELD == "ZZVCRLSH": #reel-sheet-code
				multiplierKeys.append(" ".ljust(line.LENGTH))
			elif line.SAP_FIELD == "HIENR": #Customer
				multiplierKeys.append(" ".zfill(line.LENGTH))
		else: #we have already started building variable keys
			temp = []
			for myKey in multiplierKeys:
				Trace.Write("[TRACE] 2 " + line.SAP_FIELD)
				if line.SAP_FIELD == "ZZVCENDUSEOBJCT": #end use object
					temp.append(endObj.ljust(line.LENGTH) + myKey)
				elif line.SAP_FIELD == "VKORG": #sales organisation
					temp.append(sOrg + myKey)
				elif line.SAP_FIELD == "VTWEG": #Distribution Channel
					temp.append(distCh + myKey)
				elif line.SAP_FIELD == "SPART": #Division
					temp.append(divOrg + myKey)
				elif line.SAP_FIELD == "KUNNR": #Sold-to party
					temp.append(soldTos.zfill(line.LENGTH) + myKey)
				elif line.SAP_FIELD == "ZZCUSZS": #Sales company
					temp.append(soldTos.zfill(line.LENGTH) + myKey)
				elif line.SAP_FIELD == "ZZHIEZU01": #CustomerHierarchy01
					temp.append(" ".zfill(line.LENGTH) + myKey)
				elif line.SAP_FIELD == "ZZVCSBRAND2": #sales brand 2
					temp.append(" ".ljust(line.LENGTH) + myKey)
				elif line.SAP_FIELD == "ZZVCRLSH": #reel-sheet-code
					temp.append(" ".ljust(line.LENGTH) + myKey)
				elif line.SAP_FIELD == "HIENR": #Customer
					temp.append(" ".zfill(line.LENGTH) + myKey)
	...
	for myKey in multiplierKeys:
			conditionKey.append(getConditionKey(tableNum, condType, myKey, sOrg, distCh, divOrg))
...
def getJson(Quote, refDocNum):
    ...
    for attr in Quote.GetCustomField('BO_CF_RCPT_OPT').AttributeValues:
        if attr.DisplayValue == Quote.GetCustomField('BO_CF_RCPT_OPT').Content:
            ...
            break
    #------------------------------------------------------------------------------
    if recipientType == "1" or recipientType == "" : #rebate does not have customer group
    ...
    #get keys-----------------------------------------------------------------------
        ...
        if len(matTypes) > 0:
            isRlSh = True
        if soldTos.Rows.Count > 0:
            isCust = True
        tableNum = getRebateCondition(condType, agrType, isEndObj, isSb2, isGroup, isRlSh, isCust)
        ...
        if isRlSh:
                    ...
                    for brand in brands:
                        for soldTo in soldTos.Rows:
                            ...
                            if agrType == "3": #end customer
                                ...
                                variableKey = salesOrg + recipient + brand.ljust(6) + soldTo["SAPID"].zfill(10) + matType
                                if isEndObj:
                                    for endObj in endObjs:
                                        #add end use object if applicable
                                        if isEndObj:
                                            variableKey = salesOrg + recipient + brand.ljust(6) + soldTo["SAPID"].zfill(10)

                                            #build dictionary for condition key [UPDATE]
                        ...
                        if agrType == "3": #end customer
                            ...
                            if isEndObj:
                                for endObj in endObjs:
                                    #add end use object if applicable
                                    if isEndObj:
                                        variableKey = salesOrg + recipient + matType + soldTo["SAPID"].zfill(10) + endObj
            ...
            if isSb2:
                for brand in brands:
                    for soldTo in soldTos.Rows:
                        variableKey = salesOrg
                        variableKey =  variableKey + brand.ljust(6) + soldTo["SAPID"].zfill(10)
                        #add end customer if applicable
                        if agrType == "3": #end customer
                            #Sales org/Distr ch/Div/Sold-to/SB2/RL-SH/End User
                            variableKey = salesOrg + recipient + brand.ljust(6) + soldTo["SAPID"].zfill(10)
                            if isEndObj:
                                for endObj in endObjs:
                                    #add end use object if applicable
                                    if isEndObj:
                                        variableKey = salesOrg + recipient + brand.ljust(6) + soldTo["SAPID"].zfill(10) + endObj
                                        #build dictionary for condition key [UPDATE]
                                        condKey = getConditionKey(usage,		#Usage of the condition table; constant "E"
                                                                tableNum,	  	#condition table
                                                                app,		  	#Application; constant"V"
                ...
                for soldTo in soldTos.Rows:
                    variableKey = salesOrg
                    variableKey =  variableKey + soldTo["SAPID"].zfill(10)
                    #add end customer if applicable
                    if agrType == "3": #end customer
                        #Sales org/Distr ch/Div/Sold-to/SB2/RL-SH/End User
                        variableKey = salesOrg + recipient + soldTo["SAPID"].zfill(10)
                        if isEndObj:
                            for endObj in endObjs:
                                #add end use object if applicable
                                if isEndObj:
                                    variableKey = salesOrg + recipient + soldTo["SAPID"].zfill(10) + endObj
...
#-------------------------------------------------------------------------------
        #build dictionary for condition key [CREATE]
        condKey = getConditionKey(usage,		#Usage of the condition table; constant "E"
                                tableNum,	  	#condition table
                                app,		  	#Application; constant"V"
                                condType, 	#condition type
